...
The relative significance index, \( Q_i \), accommodates the inverse scaling of cost variables by embedding them in the harmonic sum proportion. In the general case comprising both benefit and cost criteria:
\begin{equation}
    Q_i = S_i^+ + \frac{\sum_{k=1}^{m} S_k^-}{S_i^- \sum_{k=1}^{m} \frac{1}{S_k^-}}
\end{equation}
where the second term penalizes poor performance on cost-oriented sub-criteria.

In the context of the \PAPI framework, where all sub-criteria are benefit-oriented (\( \mathcal{C} = \emptyset \)), the cost-component sums equal zero: \( S_i^- = 0 \) for all provinces \( i \). Consequently, the cost penalty term vanishes and COPRAS reduces to:
\begin{equation}
...
which averages all $\binom{6}{2} = 15$ pairwise correlations across the five core methods and baseline.
